...
angular.module('bk-page-account', ['ui.state']).config([
  ...
  'rGroups',
  'Session',
  function Account($scope, List, rGroups, Session) {
...
;
angular.module('bk-page-home', ['ui.state']).config([
  '$stateProvider',
  function config($stateProvider) {
      ...
      resolve: {
        rList: [
          ...
          function (List, $stateParams) {
            ...
            return List.getGroupItems(group).then(function (res) {
              return res.items;
            }, function (err) {
              console.log(err);
            });
  ...
  '$stateParams',
  'rList',
  function HomeCtrl($scope, List, $stateParams, rList) {
...
angular.module('bk-page-item', ['ui.state']).config([
  '$stateProvider',
  function config($stateProvider) {
      ...
      resolve: {
        rItem: [
          ...
          function ($stateParams, Item) {
            console.log($stateParams);
            return Item.getItem($stateParams.ref).then(function (res) {
              console.log(res);
              return res.item;
            }, function (err) {
              console.log(err);
  ...
  'rItem',
  '$http',
  function ItemCtrl($scope, rItem, $http) {
    console.log(rItem);
    $scope.item = rItem;
    $scope.sendMessage = function () {
      ...
      $http.post('/1.0/message', outbound).success(function (err, res) {
        console.log(res === 200);
        if (res === 200) {
          $scope.success = true;
        }
...
angular.module('bk-page-login', ['ui.state']).config([
  '$stateProvider',
  function config($stateProvider) {
  ...
  '$rootScope',
  'Session',
  function LoginCtrl($scope, User, $state, $http, $rootScope, Session) {
    $scope.loginUser = function () {
      User.login($scope.user.username, $scope.user.pass).then(function (res) {
...
angular.module('bk-page-signup', ['ui.state']).config([
  ...
  'User',
  '$state',
  function SignupCtrl($scope, User, $state) {
...
angular.module('bk-service-item', []).factory('Item', [
  '$http',
  function ($http) {
    return {
      getItem: function (ref) {
        return $http.get('/1.0/item/' + ref).then(function (result) {
          return result.data;
        });
      }
    };
...
angular.module('bk-service-list', []).factory('List', [
  '$http',
  function ($http) {
    return {
      getGroupItems: function (group) {
        return $http.get('/1.0/items/?group=' + group).then(function (result) {
          return result.data;
        });
      },
      getGroups: function () {
        return $http.get('/1.0/groups').then(function (result) {
          return result.data;
        });
      }
    };
...
angular.module('bk-service-session', ['ngCookies']).factory('Session', [
  '$cookieStore',
...
angular.module('bk-service-user', []).factory('User', [
  '$http',
  'Session',
  '$rootScope',
  '$state',
  function ($http, Session, $rootScope, $state) {
    return {
      signup: function (data) {
        return $http.post('/1.0/user', data).then(function (result) {
          return result.data;
        });
      },
      login: function (user, pass) {
        return $http.post('/1.0/login', {
          username: user,
          pass: pass
        }).then(function (result) {
          return result.data;
      ...
      autologin: function (redirect) {
        return $http.get('/1.0/autologin', { headers: { token: Session.getToken() } }).then(function (result) {
          return result.data;
        }).then(function (data) {
          $http.defaults.headers.common['token'] = Session.getToken();
          $rootScope.state = 'loggedIn';
...
angular.module("home/home.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("home/home.tpl.html",
    ...
    "        </div>\n" +
    "        <div class=\"forms\">\n" +
    "            <form ng-show=\"type === '1st'\">\n" +
    "                All\n" +
    "            </form>\n" +
    "            <form ng-show=\"type === '2nd'\">\n" +
    "                Textbooks\n" +
    "            </form>\n" +
    "            <form ng-show=\"type === '3rd'\">\n" +
    "                Furniture\n" +
    "            </form>\n" +
    "        </div>\n" +
    "        <!--<div class=\"actuator\">-->\n" +
...
angular.module("item/item.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("item/item.tpl.html",
    ...
    "            <textarea cols=\"23\" rows=\"10\" ng-model=\"message.body\">\n" +
    ...
    "                Thank you.\n" +
    "            </textarea>\n" +
    "            <button ng-click=\"sendMessage();\" class=\"btn btn--gray\">\n" +
    "                Send Reply\n" +
    "            </button>\n" +
    "        </aside>\n" +
    "        <section ng-show=\"success === true\">\n" +
    "            <h3>Message sent!</h3>\n" +
